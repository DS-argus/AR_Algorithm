...
/*-------------------- Desktop --------------------*/
void Desktop::AddGPU(Computer *aGPU) {
    if (nGPU == 0)
       GPU1 = aGPU;
    else
       GPU2 = aGPU;
...
float Desktop::Price(void) {
    if (nGPU == 0){
        return(Computer::Price());
    }
    else {
        return(Computer::Price() * 1.8);
    }
}

float Desktop::TotalPrice(void){
    float GPUPrice1 = 0;
    float GPUPrice2 = 0;
...
void Desktop::PrintSelf() {
    printf("It is a Desktop with %d GPU(s), Size=%.0f, Price=%.0f, Total Price=%.0f\n", nGPU, size, Price(), TotalPrice());
}